...
#### `quarto.qmd` — The Quarto Language (~360 lines) — **frame**
**Subtitle:** "Markdown with a compiler behind it." Targets Quarto 1.9, HTML website projects first; PDF is planned later.
Written: overview (Quarto as a build pipeline, not just Markdown), a short history from literate programming through knitr/Pandoc/R Markdown with a release table, the philosophy, writing about Quarto in Quarto (`#verbatim`: escaping shortcodes, `{{python}}` cells, `echo: fenced`, nesting fences), and includes and embeds (`#includes`: the `include` shortcode, `embed`, `include-in-header` and friends). Everything else is a stub: render pipeline, front matter, Markdown, divs and spans, callouts, cross-references, code cells, diagrams, shortcodes, projects, websites, layout, styling, other formats, extensions, the CLI, pitfalls, closing sections. `_quarto-include-demo.qmd` is a live fragment included by the chapter. Shortcodes expand inside code blocks and HTML comments, so any shortcode shown in this chapter (stubs included) must be escaped.
