...
<!-- STUB: Move the precedence table here from the pitfalls section. Add prose on
     the arithmetic/relational/logical/bitwise families, the difference between
     && and & (short-circuit vs. bitwise), compound assignment, the comma
     operator, and sequence points / unsequenced modification. -->
...
### Variadic Functions
<!-- STUB: <stdarg.h>: va_list, va_start, va_arg, va_end. Why printf needs a
     format string (no runtime type information to recover the arguments), the
     default argument promotions that apply to variadic parameters, and why
     these are type-unsafe in a way C++ templates and Rust macros are not. -->
...
### Pointers {#pointers}
A **pointer** is a variable that stores the memory address of another object. Declaring one uses `*`, as does dereferencing it, which is the "declaration mirrors use" principle at work: `int *ptr` declares `ptr` such that `*ptr` is an `int`.
...
A **pointer to a pointer** is exactly what it sounds like, and it is how a function modifies a caller's *pointer* rather than the thing it points at, which is what an allocating function or a linked-list insertion needs.
...
The parentheses carry the whole distinction. `[]` and `()` bind tighter than `*`, so `int *arr[10]` groups as an array first and `int (*arr)[10]` forces the pointer to be read first. The two types are unrelated and not interchangeable. When a declaration reaches this density, the kindness to your reader is a `typedef`, which is exactly why function-pointer types are nearly always aliased before use.
...
::: {.callout-note title="Padding is not optional"}
The compiler inserts padding between members so each is properly aligned for the target, which means `sizeof` a struct is generally *more* than the sum of its members, and member order affects the total. Never assume a struct's layout when reading a wire format or a file header: read the fields individually. `__attribute__((packed))` suppresses the padding but makes member access slower, and on strict-alignment architectures it can produce unaligned accesses that fault outright.
:::
...
**Bitwise operators bind more loosely than comparison operators**, a decision Ritchie later called a mistake, retained because code already depended on it. Parenthesize.
