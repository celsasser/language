...
- For a more-or-less up-to-date to-do list see: [Unclassified proto-rules](#s-unclassified).
...
- [Classes and class hierarchies](#s-class)
- [Enumerations](#s-enum)
- [Resource management](#s-resource)
- [Expressions and statements](#s-expr)
- [Performance](#s-performance)
- [Concurrency and parallelism](#s-concurrency)
- [Error handling](#s-errors)
- [Constants and immutability](#s-const)
- [Templates and generic programming](#s-templates)
- [C-style programming](#s-cpl)
- [Source files](#s-source)
- [The Standard Library](#sl-the-standard-library)

Supporting sections:

- [Architectural ideas](#s-a)
- [Non-Rules and myths](#s-not)
- [References](#s-references)
- [Profiles](#s-profile)
- [Guidelines support library](#s-gsl)
- [Naming and layout suggestions](#s-naming)
- [Answers to frequently asked questions](#s-faq)
...
- [Target readership](#ss-readers)
- [Aims](#ss-aims)
- [Non-aims](#ss-non)
- [Enforcement](#ss-force)
- [The structure of this document](#ss-struct)
- [Major sections](#ss-sec)
...
- [Express ideas directly in code](#rp-direct)
- [Write in ISO Standard C++](#rp-cplusplus)
- [Express intent](#rp-what)
- [Ideally, a program should be statically type safe](#rp-typesafe)
- [Prefer compile-time checking to run-time checking](#rp-compile-time)
- [What cannot be checked at compile time should be checkable at run time](#rp-run-time)
- [Catch run-time errors early](#rp-early)
- [Don't leak any resources](#rp-leak)
- [Don't waste time or space](#rp-waste)
- [Prefer immutable data to mutable data](#rp-mutable)
- [Encapsulate messy constructs, rather than spreading through the code](#rp-library)
- [Use supporting tools as appropriate](#rp-tools)
- [Use support libraries as appropriate](#rp-lib)
...
See [Constants and immutability](#s-const)
...
- [Make interfaces explicit](#ri-explicit)
- [Avoid non-`const` global variables](#ri-global)
- [Avoid singletons](#ri-singleton)
- [Make interfaces precisely and strongly typed](#ri-typed)
- [State preconditions (if any)](#ri-pre)
- [Prefer `Expects()` for expressing preconditions](#ri-expects)
- [State postconditions](#ri-post)
- [Prefer `Ensures()` for expressing postconditions](#ri-ensures)
- [If an interface is a template, document its parameters using concepts](#ri-concepts)
- [Use exceptions to signal a failure to perform a required task](#ri-except)
- [Never transfer ownership by a raw pointer (`T*`) or reference (`T&`)](#ri-raw)
- [Declare a pointer that must not be null as `not_null`](#ri-nullptr)
- [Do not pass an array as a single pointer](#ri-array)
- [Avoid complex initialization of global objects](#ri-global-init)
- [Keep the number of function arguments low](#ri-nargs)
- [Avoid adjacent parameters that can be invoked by the same arguments in either order with different meaning](#ri-unrelated)
- [Prefer empty abstract classes as interfaces to class hierarchies](#ri-abstract)
- [If you want a cross-compiler ABI, use a C-style subset](#ri-abi)
- [For stable library ABI, consider the Pimpl idiom](#ri-pimpl)
- [Encapsulate rule violations](#ri-encapsulate)

##### See also
- [Functions](#s-functions)
- [Concrete types](#ss-concrete)
- [Class hierarchies](#ss-hier)
- [Overloading and overloaded operators](#ss-overload)
- [Containers and other resource handles](#ss-containers)
- [Error handling](#s-errors)
- $1#Templates and generic programming](#s-templates)
...
#### Never transfer ownership by a raw pointer {#ri-raw}
::: {.subheading}
`T*`) or reference (`T&`
:::
...
- ["Package" meaningful operations as carefully named functions](#rf-package)
- [A function should perform a single logical operation](#rf-logical)
- [Keep functions short and simple](#rf-single)
- [If a function might have to be evaluated at compile time, declare it `constexpr`](#rf-constexpr)
- [If a function is very small and time-critical, declare it inline](#rf-inline)
- [If your function must not throw, declare it `noexcept`](#rf-noexcept)
- [For general use, take `T*` or `T&` arguments rather than smart pointers](#rf-smart)
- [Prefer pure functions](#rf-pure)
- [Unused parameters should be unnamed](#rf-unused)
- [If an operation can be reused, give it a name](#rf-name)
- [Use an unnamed lambda if you need a simple function object in one place only](#rf-lambda)

Parameter passing expression rules:

- [Prefer simple and conventional ways of passing information](#rf-conventional)
- [For "in" parameters, pass cheaply-copied types by value and others by reference to `const`](#rf-in)
- [For "in-out" parameters, pass by reference to non-`const`](#rf-inout)
- [For "will-move-from" parameters, pass by `X&&` and `std::move` the parameter](#rf-consume)
- [For "forward" parameters, pass by `TP&&` and only `std::forward` the parameter](#rf-forward)
- [For "out" output values, prefer return values to output parameters](#rf-out)
- [To return multiple "out" values, prefer returning a struct](#rf-out-multi)
- [Prefer `T*` over `T&` when "no argument" is a valid option](#rf-ptr-ref)

Parameter passing semantic rules:

- [Use `T*` or `owner<T*>` to designate a single object](#rf-ptr)
- [Use a `not_null<T>` to indicate that "null" is not a valid value](#rf-nullptr)
- [Use a `span<T>` or a `span_p<T>` to designate a half-open sequence](#rf-range)
- [Use a `zstring` or a `not_null<zstring>` to designate a C-style string](#rf-zstring)
- [Use a `unique_ptr<T>` to transfer ownership where a pointer is needed](#rf-unique_ptr)
- [Use a `shared_ptr<T>` to share ownership](#rf-shared_ptr)

Value return semantic rules: {#rf-value-return}

- [Return a `T*` to indicate a position (only)](#rf-return-ptr)
- [Never (directly or indirectly) return a pointer or a reference to a local object](#rf-dangle)
- [Return a `T&` when copy is undesirable and "returning no object" isn't needed](#rf-return-ref)
- [Don't return a `T&&`](#rf-return-ref-ref)
- [`int` is the return type for `main()`](#rf-main)
- [Return `T&` from assignment operators](#rf-assignment-op)
- [Don't return `std::move(local)`](#rf-return-move-local)
- [Don't return `const T`](#rf-return-const)

Other function rules:

- [Use a lambda when a function won't do (to capture local variables, or to write a local function)](#rf-capture-vs-overload)
- [Where there is a choice, prefer default arguments over overloading](#rf-default-args)
- [Prefer capturing by reference in lambdas that will be used locally, including passed to algorithms](#rf-reference-capture)
- [Avoid capturing by reference in lambdas that will be used non-locally, including returned, stored on the heap, or passed to another thread](#rf-value-capture)
- [When writing a lambda that captures `this` or any class data member, don't use `[=]` default capture](#rf-this-capture)
- [Don't use `va_arg` arguments](#f-varargs)
- [Avoid unnecessary condition nesting](#f-nesting)

Functions have strong similarities to lambdas and function objects.

**See also**: [Function objects and lambdas](#ss-lambdas)

### Function definitions {#ss-fct-def}
...
![](images/cppr-01.png){fig-alt="Normal parameter passing table" width=9in}

Advanced parameter passing:

![](images/cppr-02.png){fig-alt="Advanced parameter passing table" width=9in}
...
#### Return a `T*` to indicate a position {#rf-return-ptr}
::: {.subheading}
only
:::
...
#### Use a lambda when a function won't do {#rf-capture-vs-overload}
::: {.subheading}
...
- [Organize related data into structures (`struct`s or `class`es)](#rc-org)
- [Use `class` if the class has an invariant; use `struct` if the data members can vary independently](#rc-struct)
- [Represent the distinction between an interface and an implementation using a class](#rc-interface)
- [Make a function a member only if it needs direct access to the representation of a class](#rc-member)
- [Place helper functions in the same namespace as the class they support](#rc-helper)
- [Don't define a class or enum and declare a variable of its type in the same statement](#rc-standalone)
- [Use `class` rather than `struct` if any member is non-public](#rc-class)
...
- [Containers and other resource handles](#ss-containers)
- [Function objects and lambdas](#ss-lambdas)
- [Class hierarchies (OOP)](#ss-hier)
- [Overloading and overloaded operators](#ss-overload)
- [Unions](#ss-union)

#### Organize related data into structures {#rc-org}
::: {.subheading}
`struct`s or `class`es
:::
...
- [Prefer concrete types over class hierarchies](#rc-concrete)
- [Make concrete types regular](#rc-regular)
- [Don't make data members `const` or references in a copyable or movable type](#rc-constref)
- [If data member `B` uses another data member `A`, declare `A` before `B`](#rc-lifetime)
...
- [If you can avoid defining any default operations, do](#rc-zero)
- [If you define or `=delete` any copy, move, or destructor function, define or `=delete` them all](#rc-five)
- [Make default operations consistent](#rc-matched)

Destructor rules:

- [Define a destructor if a class needs an explicit action at object destruction](#rc-dtor)
- [All resources acquired by a class must be released by the class's destructor](#rc-dtor-release)
- [If a class has a raw pointer (`T*`) or reference (`T&`), consider whether it might be owning](#rc-dtor-ptr)
- [If a class has an owning pointer member, define a destructor](#rc-dtor-ptr2)
- [A base class destructor should be either public and virtual, or protected and non-virtual](#rc-dtor-virtual)
- [A destructor must not fail](#rc-dtor-fail)
- [Make destructors `noexcept`](#rc-dtor-noexcept)

Constructor rules:

- [Define a constructor if a class has an invariant](#rc-ctor)
- [A constructor should create a fully initialized object](#rc-complete)
- [If a constructor cannot construct a valid object, throw an exception](#rc-throw)
- [Ensure that a copyable class has a default constructor](#rc-default0)
- [Prefer default constructors to be simple and non-throwing](#rc-default00)
- [Don't define a default constructor that only initializes data members; use member initializers instead](#rc-default)
- [By default, declare single-argument constructors `explicit`](#rc-explicit)
- [Define and initialize data members in the order of member declaration](#rc-order)
- [Prefer default member initializers to member initializers in constructors for constant initializers](#rc-in-class-initializer)
- [Prefer initialization to assignment in constructors](#rc-initialize)
- [Use a factory function if you need "virtual behavior" during initialization](#rc-factory)
- [Use delegating constructors to represent common actions for all constructors of a class](#rc-delegating)
- [Use inheriting constructors to import constructors into a derived class that does not need further explicit initialization](#rc-inheriting)

Copy and move rules:

- [Make copy assignment non-`virtual`, take the parameter by `const&`, and return by non-`const&`](#rc-copy-assignment)
- [A copy operation should copy](#rc-copy-semantic)
- [Make copy assignment safe for self-assignment](#rc-copy-self)
- [Make move assignment non-`virtual`, take the parameter by `&&`, and return by non-`const&`](#rc-move-assignment)
- [A move operation should move and leave its source in a valid state](#rc-move-semantic)
- [Make move assignment safe for self-assignment](#rc-move-self)
- [Make move operations `noexcept`](#rc-move-noexcept)
- [A polymorphic class should suppress public copy/move](#rc-copy-virtual)

Other default operations rules:

- [Use `=default` if you have to be explicit about using the default semantics](#rc-eqdefault)
- [Use `=delete` when you want to disable default behavior (without wanting an alternative)](#rc-delete)
- [Don't call virtual functions in constructors and destructors](#rc-ctor-virtual)
- [For value-like types, consider providing a `noexcept` swap function](#rc-swap)
- [A `swap` must not fail](#rc-swap-fail)
- [Make `swap` `noexcept`](#rc-swap-noexcept)
- [Make `==` symmetric with respect of operand types and `noexcept`](#rc-eq)
- [Beware of `==` on base classes](#rc-eq-base)
- [Make a `hash` `noexcept`](#rc-hash)
- [Rely on constructors and assignment operators, not memset and memcpy](#rc-memset)

### Default Operations {#ss-defop}
...
#### If you can avoid defining default operations, do {#rc-zero}
##### Reason
...
#### Don't define a default constructor that only initializes data members; use default member initializers instead {#rc-default}
##### Reason
...
#### Use `=delete` when you want to disable default behavior {#rc-delete}
::: {.subheading}
...
#### A `swap` function must not fail {#rc-swap-fail}
##### Reason
...
#### Make `==` symmetric with respect to operand types and `noexcept` {#rc-eq}
##### Reason
...
- [Follow the STL when defining a container](#rcon-stl)
- [Give a container value semantics](#rcon-val)
- [Give a container move operations](#rcon-move)
- [Give a container an initializer list constructor](#rcon-init)
- [Give a container a default constructor that sets it to empty](#rcon-empty)
- [If a resource handle has pointer semantics, provide `*` and `->`](#rcon-ptr)
...
- [Prefer capturing by reference in lambdas that will be used locally, including passed to algorithms](#rf-reference-capture)
- [Avoid capturing by reference in lambdas that will be used non-locally, including returned, stored on the heap, or passed to another thread](#rf-value-capture)
- [Use lambdas for complex initialization, especially of `const` variables](#res-lambda-init)
...
- [Use class hierarchies to represent concepts with inherent hierarchical structure (only)](#rh-domain)
- [If a base class is used as an interface, make it a pure abstract class](#rh-abstract)
- [Use abstract classes as interfaces when complete separation of interface and implementation is needed](#rh-separation)

Designing rules for classes in a hierarchy summary:

- [An abstract class typically doesn't need a user-written constructor](#rh-abstract-ctor)
- [A class with a virtual function should have a virtual or protected destructor](#rh-dtor)
- [Virtual functions should specify exactly one of `virtual`, `override`, or `final`](#rh-override)
- [When designing a class hierarchy, distinguish between implementation inheritance and interface inheritance](#rh-kind)
- [For making deep copies of polymorphic classes prefer a virtual `clone` function instead of public copy construction/assignment](#rh-copy)
- [Avoid trivial getters and setters](#rh-get)
- [Don't make a function `virtual` without reason](#rh-virtual)
- [Avoid `protected` data](#rh-protected)
- [Ensure all non-`const` data members have the same access level](#rh-public)
- [Use multiple inheritance to represent multiple distinct interfaces](#rh-mi-interface)
- [Use multiple inheritance to represent the union of implementation attributes](#rh-mi-implementation)
- [Use `virtual` bases to avoid overly general base classes](#rh-vbase)
- [Create an overload set for a derived class and its bases with `using`](#rh-using)
- [Use `final` on classes sparingly](#rh-final)
- [Do not provide different default arguments for a virtual function and an overrider](#rh-virtual-default-arg)

Accessing objects in a hierarchy rule summary:

- [Access polymorphic objects through pointers and references](#rh-poly)
- [Use `dynamic_cast` where class hierarchy navigation is unavoidable](#rh-dynamic_cast)
- [Use `dynamic_cast` to a reference type when failure to find the required class is considered an error](#rh-ref-cast)
- [Use `dynamic_cast` to a pointer type when failure to find the required class is considered a valid alternative](#rh-ptr-cast)
- [Use `unique_ptr` or `shared_ptr` to avoid forgetting to `delete` objects created using `new`](#rh-smart)
- [Use `make_unique()` to construct objects owned by `unique_ptr`s](#rh-make_unique)
- [Use `make_shared()` to construct objects owned by `shared_ptr`s](#rh-make_shared)
- [Never assign a pointer to an array of derived class objects to a pointer to its base](#rh-array)
- [Prefer virtual function to casting](#rh-use-virtual)

#### Use class hierarchies to represent concepts with inherent hierarchical structure {#rh-domain}
::: {.subheading}
only
:::
...
Note that we can put default initializers on data members: [Prefer initialization to assignment in constructors](#rc-initialize).
...
- [Define operators primarily to mimic conventional usage](#ro-conventional)
- [Use non-member functions for symmetric operators](#ro-symmetric)
- [Overload operations that are roughly equivalent](#ro-equivalent)
- [Overload only for operations that are roughly equivalent](#ro-equivalent-2)
- [Avoid implicit conversion operators](#ro-conversion)
- [Use `using` for customization points](#ro-custom)
- [Overload unary `&` only as part of a system of smart pointers and references](#ro-address-of)
- [Use an operator for an operation with its conventional meaning](#ro-overload)
- [Define overloaded operators in the namespace of their operands](#ro-namespace)
- [If you feel like overloading a lambda, use a generic lambda](#ro-lambda)
...
- [Use `union`s to save Memory](#ru-union)
- [Avoid "naked" `union`s](#ru-naked)
- [Use anonymous `union`s to implement tagged unions](#ru-anonymous)
- [Don't use a `union` for type punning](#ru-pun)
- ???
...
- [Prefer enumerations over macros](#renum-macro)
- [Use enumerations to represent sets of related named constants](#renum-set)
- [Prefer `enum class`es over "plain" `enum`s](#renum-class)
- [Define operations on enumerations for safe and simple use](#renum-oper)
- [Don't use `ALL_CAPS` for enumerators](#renum-caps)
- [Avoid unnamed enumerations](#renum-unnamed)
- [Specify the underlying type of an enumeration only when necessary](#renum-underlying)
- [Specify enumerator values only when necessary](#renum-value)
...
#### Prefer class enums over "plain" enums {#renum-class}
##### Reason
...
- Resource management rule summary:
   - [Manage resources automatically using resource handles and RAII (Resource Acquisition Is Initialization)](#rr-raii)
   - [In interfaces, use raw pointers to denote individual objects (only)](#rr-use-ptr)
   - [A raw pointer (a `T*`) is non-owning](#rr-ptr)
   - [A raw reference (a `T&`) is non-owning](#rr-ref)
   - [Prefer scoped objects, don't heap-allocate unnecessarily](#rr-scoped)
   - [Avoid non-`const` global variables](#rr-global)

- Allocation and deallocation rule summary:
   - [Avoid `malloc()` and `free()`](#rr-mallocfree)
   - [Avoid calling `new` and `delete` explicitly](#rr-newdelete)
   - [Immediately give the result of an explicit resource allocation to a manager object](#rr-immediate-alloc)
   - [Perform at most one explicit resource allocation in a single expression statement](#rr-single-alloc)
   - [Avoid `[]` parameters, prefer `span`](#rr-ap)
   - [Always overload matched allocation/deallocation pairs](#rr-pair)

- Smart pointer rule summary: {#rr-summary-smartptrs}
   - [Use `unique_ptr` or `shared_ptr` to represent ownership](#rr-owner)
   - [Prefer `unique_ptr` over `shared_ptr` unless you need to share ownership](#rr-unique)
   - [Use `make_shared()` to make `shared_ptr`s](#rr-make_shared)
   - [Use `make_unique()` to make `unique_ptr`s](#rr-make_unique)
   - [Use `std::weak_ptr` to break cycles of `shared_ptr`s](#rr-weak_ptr)
   - [Take smart pointers as parameters only to explicitly express lifetime semantics](#rr-smartptrparam)
   - [If you have non-`std` smart pointers, follow the basic pattern from `std`](#rr-smart)
   - [Take a `unique_ptr<widget>` parameter to express that a function assumes ownership of a `widget`](#rr-uniqueptrparam)
   - [Take a `unique_ptr<widget>&` parameter to express that a function reseats the `widget`](#rr-reseat)
   - [Take a `shared_ptr<widget>` parameter to express shared ownership](#rr-sharedptrparam-owner)
   - [Take a `shared_ptr<widget>&` parameter to express that a function might reseat the shared pointer](#rr-sharedptrparam)
   - [Take a `const shared_ptr<widget>&` parameter to express that it might retain a reference count to the object ???](#rr-sharedptrparam-const)
   - [Do not pass a pointer or reference obtained from an aliased smart pointer](#rr-smartptrget)

#### Manage resources automatically using resource handles and RAII {#rr-raii}
::: {.subheading}
...
#### In interfaces, use raw pointers to denote individual objects {#rr-use-ptr}
::: {.subheading}
only
:::
...
### Allocation and deallocation {#ss-alloc}
#### Avoid `malloc()` and `free()` {#rr-mallocfree}
##### Reason
...
For naming, commenting, and indentation rules, see [Naming and layout](#s-naming).

General rules:

- [Prefer the standard library to other libraries and to "handcrafted code"](#res-lib)
- [Prefer suitable abstractions to direct use of language features](#res-abstr)
- [Don't repeat yourself, avoid redundant code](#res-dry)

Declaration rules:

- [Keep scopes small](#res-scope)
- [Declare names in for-statement initializers and conditions to limit scope](#res-cond)
- [Keep common and local names short, and keep uncommon and non-local names longer](#res-name-length)
- [Avoid similar-looking names](#res-name-similar)
- [Avoid `ALL_CAPS` names](#res-not-caps)
- [Declare one name (only) per declaration](#res-name-one)
- [Use `auto` to avoid redundant repetition of type names](#res-auto)
- [Do not reuse names in nested scopes](#res-reuse)
- [Always initialize an object](#res-always)
- [Don't introduce a variable (or constant) before you need to use it](#res-introduce)
- [Don't declare a variable until you have a value to initialize it with](#res-init)
- [Prefer the `{}`-initializer syntax](#res-list)
- [Use a `unique_ptr<T>` to hold pointers](#res-unique)
- [Declare an object `const` or `constexpr` unless you want to modify its value later on](#res-const)
- [Don't use a variable for two unrelated purposes](#res-recycle)
- [Use `std::array` or `stack_array` for arrays on the stack](#res-stack)
- [Use lambdas for complex initialization, especially of `const` variables](#res-lambda-init)
- [Don't use macros for program text manipulation](#res-macros)
- [Don't use macros for constants or "functions"](#res-macros2)
- [Use `ALL_CAPS` for all macro names](#res-all_caps)
- [If you must use macros, give them unique names](#res-macros3)
- [Don't define a (C-style) variadic function](#res-ellipses)

Expression rules:

- [Avoid complicated expressions](#res-complicated)
- [If in doubt about operator precedence, parenthesize](#res-parens)
- [Keep use of pointers simple and straightforward](#res-ptr)
- [Avoid expressions with undefined order of evaluation](#res-order)
- [Don't depend on order of evaluation of function arguments](#res-order-fct)
- [Avoid "magic constants"; use symbolic constants](#res-magic)
- [Avoid narrowing conversions](#res-narrowing)
- [Use `nullptr` rather than `0` or `NULL`](#res-nullptr)
- [Avoid casts](#res-casts)
- [If you must use a cast, use a named cast](#res-casts-named)
- [Don't cast away `const`](#res-casts-const)
- [Avoid the need for range checking](#res-range-checking)
- [Write `std::move()` only when you need to explicitly move an object to another scope](#res-move)
- [Avoid `new` and `delete` outside resource management functions](#res-new)
- [Delete arrays using `delete[]` and non-arrays using `delete`](#res-del)
- [Don't compare pointers into different arrays](#res-arr2)
- [Don't slice](#res-slice)
- [Use the `T{e}`notation for construction](#res-construct)
- [Don't dereference an invalid pointer](#res-deref)

Statement rules:

- [Prefer a `switch`-statement to an `if`-statement when there is a choice](#res-switch-if)
- [Prefer a range-`for`-statement to a `for`-statement when there is a choice](#res-for-range)
- [Prefer a `for`-statement to a `while`-statement when there is an obvious loop variable](#res-for-while)
- [Prefer a `while`-statement to a `for`-statement when there is no obvious loop variable](#res-while-for)
- [Prefer to declare a loop variable in the initializer part of a `for`-statement](#res-for-init)
- [Avoid `do`-statements](#res-do)
- [Avoid `goto`](#res-goto)
- [Minimize the use of `break` and `continue` in loops](#res-continue)
- [Don't rely on implicit fallthrough in `switch` statements](#res-break)
- [Use `default` to handle common cases (only)](#res-default)
- [Don't try to declare a local variable with no name](#res-noname)
- [Make empty statements visible](#res-empty)
- [Avoid modifying loop control variables inside the body of raw for-loops](#res-loop-counter)
- [Don't add redundant `==` or `!=` to conditions](#res-if)

Arithmetic rules:

- [Don't mix signed and unsigned arithmetic](#res-mix)
- [Use unsigned types for bit manipulation](#res-unsigned)
- [Use signed types for arithmetic](#res-signed)
- [Don't overflow](#res-overflow)
- [Don't underflow](#res-underflow)
- [Don't divide by integer zero](#res-zero)
- [Don't try to avoid negative values by using `unsigned`](#res-nonnegative)
- [Don't use `unsigned` for subscripts, prefer `gsl::index`](#res-subscripts)
...
##### C++17 and C++20 enforcement
::: {.subheading}
if using a C++17 or C++20 compiler
:::
...
#### Avoid lossy (narrowing, truncating) arithmetic conversions {#res-narrowing}
##### Reason
...
**See also**: [Resource management](#s-resource)
...
#### Use `default` to handle common cases {#res-default}
::: {.subheading}
only
:::
...
- [Don't optimize without reason](#rper-reason)
- [Don't optimize prematurely](#rper-knuth)
- [Don't optimize something that's not performance critical](#rper-critical)
- [Don't assume that complicated code is necessarily faster than simple code](#rper-simple)
- [Don't assume that low-level code is necessarily faster than high-level code](#rper-low)
- [Don't make claims about performance without measurements](#rper-measure)
- [Design to enable optimization](#rper-efficiency)
- [Rely on the static type system](#rper-type)
- [Move computation from run time to compile time](#rper-comp)
- [Eliminate redundant aliases](#rper-alias)
- [Eliminate redundant indirections](#rper-indirect)
- [Minimize the number of allocations and deallocations](#rper-alloc)
- [Do not allocate on a critical branch](#rper-alloc0)
- [Use compact data structures](#rper-compact)
- [Declare the most used member of a time-critical struct first](#rper-struct)
- [Space is time](#rper-space)
- [Access memory predictably](#rper-access)
- [Avoid context switches on the critical path](#rper-context)
...
- [Assume that your code will run as part of a multi-threaded program](#rconc-multi)
- [Avoid data races](#rconc-races)
- [Minimize explicit sharing of writable data](#rconc-data)
- [Think in terms of tasks, rather than threads](#rconc-task)
- [Don't try to use `volatile` for synchronization](#rconc-volatile)
- [Whenever feasible use tools to validate your concurrent code](#rconc-tools)

##### See also
- [Concurrency](#sscp-con)
- [Coroutines](#sscp-coro)
- [Parallelism](#sscp-par)
- [Message passing](#sscp-mess)
- [Vectorization](#sscp-vec)
- [Lock-free programming](#sscp-free)
- [Etc. concurrency rules](#sscp-etc)
...
See also [Message Passing](#sscp-mess) and [prefer pass by value](#rconc-data-by-value).
...
- [Use RAII, never plain `lock()`/`unlock()`](#rconc-raii)
- [Use `std::lock()` or `std::scoped_lock` to acquire multiple `mutex`es](#rconc-lock)
- [Never call unknown code while holding a lock (e.g., a callback)](#rconc-unknown)
- [Think of a joining `thread` as a scoped container](#rconc-join)
- [Think of a `thread` as a global container](#rconc-detach)
- [Prefer `gsl::joining_thread` over `std::thread`](#rconc-joining_thread)
- [Don't `detach()` a thread](#rconc-detached_thread)
- [Pass small amounts of data between threads by value, rather than by reference or pointer](#rconc-data-by-value)
- [To share ownership between unrelated `thread`s use `shared_ptr`](#rconc-shared)
- [Minimize context switching](#rconc-switch)
- [Minimize thread creation and destruction](#rconc-create)
- [Don't `wait` without a condition](#rconc-wait)
- [Minimize time spent in a critical section](#rconc-time)
- [Remember to name your `lock_guard`s and `unique_lock`s](#rconc-name)
- [Define a `mutex` together with the data it guards. Use `synchronized_value<T>` where possible](#rconc-mutex)
...
#### Never call unknown code while holding a lock {#rconc-unknown}
::: {.subheading}
e.g., a callback
:::
...
- [Do not use capturing lambdas that are coroutines](#rcoro-capture)
- [Do not hold locks or other synchronization primitives across suspension points](#rcoro-locks)
- [Parameters to coroutines should not be passed by reference](#rcoro-reference-parameters)
...
##### Note
The same danger applies to output parameters.  [For "out" output values, prefer return values to output parameters](#rf-out) discourages output parameters.  Coroutines should avoid them entirely.
...
- [Use a `future` to return a value from a concurrent task](#rconc-future)
- [Use `async()` to spawn concurrent tasks](#rconc-async)
- message queues
- messaging libraries
...
- [Don't use lock-free programming unless you absolutely have to](#rconc-lockfree)
- [Distrust your hardware/compiler combination](#rconc-distrust)
- [Carefully study the literature](#rconc-literature)
- how/when to use atomics
- avoid starvation
- use a lock-free data structure rather than hand-crafting specific lock-free access
- [Do not write your own double-checked locking for initialization](#rconc-double)
- [Use a conventional pattern if you really need double-checked locking](#rconc-double-pattern)
...
#### Use a conventional pattern if you really need double-checked locking {#rconc-double-pattern}
##### Reason
Double-checked locking is easy to mess up. If you really need to write your own double-checked locking, in spite of the rules [Do not write your own double-checked locking for initialization](#rconc-double) and [Don't use lock-free programming unless you absolutely have to](#rconc-lockfree), then do it in a conventional pattern.

The uses of the double-checked locking pattern that are not in violation of [Do not write your own double-checked locking for initialization](#rconc-double) arise when a non-thread-safe action is both hard and rare, and there exists a fast thread-safe test that can be used to guarantee that the action is not needed, but cannot be used to guarantee the converse.

##### Example, bad
The use of volatile does not make the first check thread-safe, see also [Use `volatile` only to talk to non-C++ memory](#rconc-volatile2)
...
- [Develop an error-handling strategy early in a design](#re-design)

- [Throw an exception to signal that a function can't perform its assigned task](#re-throw)

- [Use exceptions for error handling only](#re-errors)

- [Design your error-handling strategy around invariants](#re-design-invariants)

- [Let a constructor establish an invariant, and throw if it cannot](#re-invariant)

- [Use RAII to prevent leaks](#re-raii)

- [State your preconditions](#re-precondition)

- [State your postconditions](#re-postcondition)

- [Use `noexcept` when exiting a function because of a `throw` is impossible or unacceptable](#re-noexcept)

- [Never throw while being the direct owner of an object](#re-never-throw)

- [Use purpose-designed user-defined types as exceptions (not built-in types)](#re-exception-types)

- [Throw by value, catch exceptions from a hierarchy by reference](#re-exception-ref)

- [Destructors, deallocation, `swap`, and exception type copy/move construction must never fail](#re-never-fail)

- [Don't try to catch every exception in every function](#re-not-always)

- [Minimize the use of explicit `try`/`catch`](#re-catch)

- [Use a `final_action` object to express cleanup if no suitable resource handle is available](#re-finally)

- [If you can't throw exceptions, simulate RAII for resource management](#re-no-throw-raii)

- [If you can't throw exceptions, consider failing fast](#re-no-throw-crash)

- [If you can't throw exceptions, use error codes systematically](#re-no-throw-codes)

- [Avoid error handling based on global state (e.g. `errno`)](#re-no-throw)

- [Don't use exception specifications](#re-specifications)

- [Properly order your `catch`-clauses](#re_catch)
...
#### Use purpose-designed user-defined types as exceptions {#re-exception-types}
::: {.subheading}
not built-in types
:::
...
#### Avoid error handling based on global state {#re-no-throw}
::: {.subheading}
e.g. `errno`
:::
...
- [By default, make objects immutable](#rconst-immutable)
- [By default, make member functions `const`](#rconst-fct)
- [By default, pass pointers and references to `const`s](#rconst-ref)
- [Use `const` to define objects with values that do not change after construction](#rconst-const)
- [Use `constexpr` for values that can be computed at compile time](#rconst-constexpr)
...
- [Use templates to raise the level of abstraction of code](#rt-raise)
- [Use templates to express algorithms that apply to many argument types](#rt-algo)
- [Use templates to express containers and ranges](#rt-cont)
- [Use templates to express syntax tree manipulation](#rt-expr)
- [Combine generic and OO techniques to amplify their strengths, not their costs](#rt-generic-oo)

Concept use rule summary:

- [Specify concepts for all template arguments](#rt-concepts)
- [Whenever possible use standard concepts](#rt-std-concepts)
- [Prefer concept names over `auto` for local variables](#rt-auto)
- [Prefer the shorthand notation for simple, single-type argument concepts](#rt-shorthand)
- ???

Concept definition rule summary:

- [Avoid "concepts" without meaningful semantics](#rt-low)
- [Require a complete set of operations for a concept](#rt-complete)
- [Specify axioms for concepts](#rt-axiom)
- [Differentiate a refined concept from its more general case by adding new use patterns](#rt-refine)
- [Use tag classes or traits to differentiate concepts that differ only in semantics](#rt-tag)
- [Avoid complementary constraints](#rt-not)
- [Prefer to define concepts in terms of use-patterns rather than simple syntax](#rt-use)
- [Use concept negation (`!C<T>`) sparingly to express a minor difference](#rt-???)
- [Use concept disjunction (`C1<T> || C2<T>`) sparingly to express alternatives](#rt-???)
- ???

Template interface rule summary:

- [Use function objects to pass operations to algorithms](#rt-fo)
- [Require only essential properties in a template's concepts](#rt-essential)
- [Use template aliases to simplify notation and hide implementation details](#rt-alias)
- [Prefer `using` over `typedef` for defining aliases](#rt-using)
- [Use function templates to deduce class template argument types (where feasible)](#rt-deduce)
- [(removed)](#rt-regular)
- [Avoid highly visible unconstrained templates with common names](#rt-visible)
- [If your compiler does not support concepts, fake them with `enable_if`](#rt-concept-def)
- [Where possible, avoid type-erasure](#rt-erasure)

Template definition rule summary:

- [Minimize a template's context dependencies](#rt-depend)
- [Do not over-parameterize members (SCARY)](#rt-scary)
- [Place non-dependent class template members in a non-templated base class](#rt-nondependent)
- [Use specialization to provide alternative implementations of class templates](#rt-specialization)
- [Use tag dispatch to provide alternative implementations of functions](#rt-tag-dispatch)
- [Use specialization to provide alternative implementations for irregular types](#rt-specialization2)
- [Use `{}` rather than `()` within templates to avoid ambiguities](#rt-cast)
- [Inside a template, don't make an unqualified non-member function call unless you intend it to be a customization point](#rt-customization)

Template and hierarchy rule summary:

- [Do not naively templatize a class hierarchy](#rt-hier)
- [Do not mix hierarchies and arrays](#rt-array) // ??? somewhere in "hierarchies"
- [Linearize a hierarchy when virtual functions are undesirable](#rt-linear)
- [Do not declare a member function template virtual](#rt-virtual)
- [Use a non-template core implementation to provide an ABI-stable interface](#rt-abi)
- [T.??: ????](#rt-???)

Variadic template rule summary:

- [Use variadic templates when you need a function that takes a variable number of arguments of a variety of types](#rt-variadic)
- [??? How to pass arguments to a variadic template ???](#rt-variadic-pass)
- [??? How to process arguments to a variadic template ???](#rt-variadic-process)
- [Don't use variadic templates for homogeneous argument lists](#rt-variadic-not)
- [T.??: ????](#rt-???)

Metaprogramming rule summary:

- [Use template metaprogramming only when you really need to](#rt-metameta)
- [Use template metaprogramming primarily to emulate concepts](#rt-emulate)
- [Use templates (usually template aliases) to compute types at compile time](#rt-tmp)
- [Use `constexpr` functions to compute values at compile time](#rt-fct)
- [Prefer to use standard-library TMP facilities](#rt-std-tmp)
- [If you need to go beyond the standard-library TMP facilities, use an existing library](#rt-lib)
- [T.??: ????](#rt-???)

Other template rules summary:

- [If an operation can be reused, give it a name](#rt-name)
- [Use an unnamed lambda if you need a simple function object in one place only](#rt-lambda)
- [Use template variables to simplify notation](#rt-var)
- [Don't write unintentionally non-generic code](#rt-non-generic)
- [Don't specialize function templates](#rt-specialize-function)
- [Check that a class matches a concept using `static_assert`](#rt-check-class)
- [T.??: ????](#rt-???)

### Generic programming {#ss-gp}
Generic programming is programming using types and algorithms parameterized by types, values, and algorithms.
...
- [Specify concepts for all template arguments](#rt-concepts)
- [Whenever possible use standard concepts](#rt-std-concepts)
- [Prefer concept names over `auto`](#rt-auto)
- [Prefer the shorthand notation for simple, single-type argument concepts](#rt-shorthand)
- ???
...
- [Differentiate a refined concept from its more general case by adding new use patterns](#rt-refine)
- [Use tag classes or traits to differentiate concepts that differ only in semantics](#rt-tag)
- [Avoid complimentary constraints](#rt-not)
- [Prefer to define concepts in terms of use-patterns rather than simple syntax](#rt-use)
- ???

### T.con-use {#ss-concept-use}
::: {.subheading}
Concept use
:::
...
### T.concepts.def {#ss-concepts-def}
::: {.subheading}
...
### Template interfaces {#ss-temp-interface}
...
#### Use function templates to deduce class template argument types {#rt-deduce}
::: {.subheading}
where feasible
:::
...
##### Note
### Template definitions {#ss-temp-def}
...
#### Do not over-parameterize members {#rt-scary}
::: {.subheading}
SCARY
:::
...
#### Use tag dispatch to provide alternative implementations of a function {#rt-tag-dispatch}
##### Reason
- A template defines a general interface.
...
#### Do not mix hierarchies and arrays {#rt-array}
##### Reason
...
### Variadic template rules {#ss-variadic}
???
...
### Template metaprogramming {#ss-meta}
::: {.subheading}
TMP
:::
...
### Other template rules {#ss-temp-other}
#### If an operation can be reused, give it a name {#rt-name}
See [F.10](#rf-name)
...
- [Prefer C++ to C](#rcpl-c)
- [If you must use C, use the common subset of C and C++, and compile the C code as C++](#rcpl-subset)
- [If you must use C for interfaces, use C++ in the calling code using such interfaces](#rcpl-interface)
...
- [Use a `.cpp` suffix for code files and `.h` for interface files if your project doesn't already follow another convention](#rs-file-suffix)

- [A header file must not contain object definitions or non-inline function definitions](#rs-inline)

- [Use header files for all declarations used in multiple source files](#rs-declaration-header)

- [Include header files before other declarations in a file](#rs-include-order)

- [A `.cpp` file must include the header file(s) that defines its interface](#rs-consistency)

- [Use `using namespace` directives for transition, for foundation libraries (such as `std`), or within a local scope (only)](#rs-using)

- [Don't write `using namespace` at global scope in a header file](#rs-using-directive)

- [Use `#include` guards for all header files](#rs-guards)

- [Avoid cyclic dependencies among source files](#rs-cycles)

- [Avoid dependencies on implicitly `#include`d names](#rs-implicit)

- [Header files should be self-contained](#rs-contained)

- [Prefer the quoted form of `#include` for files relative to the including file and the angle bracket form everywhere else](#rs-incform)

- [Use portable header identifiers in `#include` statements](#rs-portable-header-id)

- [Use `namespace`s to express logical structure](#rs-namespace)

- [Don't use an unnamed (anonymous) namespace in a header](#rs-unnamed)

- [Use an unnamed (anonymous) namespace for all internal/non-exported entities](#rs-unnamed2)
...
#### Use `using namespace` directives for transition, for foundation libraries {#rs-using}
::: {.subheading}
...
- [Regex](#ss-regex)
- [Time](#ss-chrono)
- [The C Standard Library](#ss-clib)

Standard-library rule summary:

- [Use libraries wherever possible](#rsl-lib)
- [Prefer the standard library to other libraries](#rsl-sl)
- [Do not add non-standard entities to namespace `std`](#sl-std)
- [Use the standard library in a type-safe manner](#sl-safe)
- ???
...
- [Prefer using STL `array` or `vector` instead of a C array](#rsl-arrays)
- [Prefer using STL `vector` by default unless you have a reason to use a different container](#rsl-vector)
- [Avoid bounds errors](#rsl-bounds)
- [don't use `memset` or `memcpy` for arguments that are not trivially-copyable](#rsl-copy)
...
#### SL.con.4 {#rsl-copy}
::: {.subheading}
don't use `memset` or `memcpy` for arguments that are not trivially-copyable
:::
...
- [Use `std::string` to own character sequences](#rstr-string)

- [Use `std::string_view` or `gsl::span<char>` to refer to character sequences](#rstr-view)

- [Use `zstring` or `czstring` to refer to a C-style, zero-terminated, sequence of characters](#rstr-zstring)

- [Use `char*` to refer to a single character](#rstr-charp)

- [Use `std::byte` to refer to byte values that do not necessarily represent characters](#rstr-byte)

- [Use `std::string` when you need to perform locale-sensitive string operations](#rstr-locale)

- [Use `gsl::span<char>` rather than `std::string_view` when you need to mutate a string](#rstr-span)

- [Use the `s` suffix for string literals meant to be standard-library `string`s](#rstr-s)

##### See also
- [F.24 span](#rf-range)
- [F.25 zstring](#rf-zstring)

#### SL.str.1 {#rstr-string}
::: {.subheading}
...
#### SL.str.11 {#rstr-span}
::: {.subheading}
Use `gsl::span<char>` rather than `std::string_view` when you need to mutate a string
:::
...
##### Discussion
::: {.subheading}
`iostream`s vs. the `printf()` family
:::
...
- [Don't use setjmp/longjmp](#rclib-jmp)
- [???](#???)
- [???](#???)
...
- [Separate stable code from less stable code](#ra-stable)
- [Express potentially reusable parts as a library](#ra-lib)
- [There should be no cycles among libraries](#ra-dag)
- [???](#???)
- [???](#???)
- [???](#???)
- [???](#???)
...
- [Don't insist that all declarations should be at the top of a function](#rnr-top)
- [Don't insist on having only a single `return`-statement in a function](#rnr-single-return)
- [Don't avoid exceptions](#rnr-no-exceptions)
- [Don't insist on placing each class definition in its own source file](#rnr-lots-of-files)
- [Don't use two-phase initialization](#rnr-two-phase-init)
- [Don't place all cleanup actions at the end of a function and `goto exit`](#rnr-goto-exit)
- [Don't make data members `protected`](#rnr-protected-data)
- ???
...
- [Coding rules](#ss-rules)
- [Books with coding guidelines](#ss-books)
- [RF.C++: C++ Programming (C++11/C++14/C++17)](#ss-cplusplus)
- [Websites](#ss-web)
- [Videos about "modern C++"](#ss-vid)
- [Manuals](#ss-man)
- [Core Guidelines materials](#ss-core)
...
- [BDE C++ Coding](https://github.com/bloomberg/bde/wiki/CodingStandards.pdf). Has a strong emphasis on code organization and layout.
- Facebook: ???
...
- [C++ Coding Standard](https://www.possibility.com/Cpp/CppCodingStandard.html). ???.
- [Secure C++ Coding Standard](https://wiki.sei.cmu.edu/confluence/x/Wnw-BQ). A very nicely done set of rules (with examples and rationales) done for security-sensitive code. Many of their rules apply generally.
...
- Stroustrup: [Style Guide](https://www.stroustrup.com/Programming/PPP-style.pdf) for [Principles and Practice using C++](https://www.stroustrup.com/programming.html). Mostly low-level naming and layout rules. Primarily a teaching tool.

### RF.C++ {#ss-cplusplus}
::: {.subheading}
C++ Programming (C++11/C++14)
:::
...
- Type.1: [Avoid casts](#res-casts): {#pro-type-avoidcasts}
   1. Don't use `reinterpret_cast`; A strict version of [Avoid casts](#res-casts) and [prefer named casts](#res-casts-named). {#pro-type-reinterpretcast}
   2. Don't use `static_cast` for arithmetic types; A strict version of [Avoid casts](#res-casts) and [prefer named casts](#res-casts-named). {#pro-type-arithmeticcast}
   3. Don't cast between pointer types where the source type and the target type are the same; A strict version of [Avoid casts](#res-casts). {#pro-type-identitycast}
   4. Don't cast between pointer types when the conversion could be implicit; A strict version of [Avoid casts](#res-casts). {#pro-type-implicitpointercast}
- Type.2: Don't use `static_cast` to downcast: {#pro-type-downcast}
   [Use `dynamic_cast` instead](#rh-dynamic_cast).
- Type.3: Don't use `const_cast` to cast away `const` (i.e., at all): {#pro-type-constcast}
   [Don't cast away const](#res-casts-const).
- Type.4: Don't use C-style `(T)expression` or functional `T(expression)` casts: {#pro-type-cstylecast}
...
- Type.6: Always initialize a data member: {#pro-type-memberinit}
   [always initialize](#res-always),
   possibly using [default constructors](#rc-default0) or
   [default member initializers](#rc-in-class-initializer).
- Type.7: Avoid naked union: {#pro-type-union}
...
### Bounds safety profile {#ss-bounds}
...
### Lifetime safety profile {#ss-lifetime}
...
The GSL is header only, and can be found at [Guidelines support library](https://github.com/Microsoft/GSL).
...
- [Views](#ss-views)
- [Ownership pointers](#ss-ownership)
- [Assertions](#ss-assertions)
- [Utilities](#ss-utilities)
- [Concepts](#ss-gsl-concepts)
...
#### Smart pointer concepts {#ss-gsl-smartptrconcepts}
...
- [Don't say in comments what can be clearly stated in code](#rl-comments)
- [State intent in comments](#rl-comments-intent)
- [Keep comments crisp](#rl-comments-crisp)
- [Maintain a consistent indentation style](#rl-indent)
- [Avoid encoding type information in names](#rl-name-type)
- [Make the length of a name roughly proportional to the length of its scope](#rl-name-length)
- [Use a consistent naming style](#rl-name)
- [Use `ALL_CAPS` for macro names only](#rl-all-caps)
- [Prefer `underscore_style` names](#rl-camel)
- [Make literals readable](#rl-literals)
- [Use spaces sparingly](#rl-space)
- [Use a conventional class member declaration order](#rl-order)
- [Use K&R-derived layout](#rl-knr)
- [Use C++-style declarator layout](#rl-ptr)
- [Avoid names that are easily misread](#rl-misread)
- [Don't place two statements on the same line](#rl-stmt)
- [Declare one name (only) per declaration](#rl-dcl)
- [Don't use `void` as an argument type](#rl-void)
- [Use conventional `const` notation](#rl-const)
- [Use a `.cpp` suffix for code files and `.h` for interface files](#rl-file-suffix)
...
written in support of Stroustrup's [Principles and Practice using C++](https://www.stroustrup.com/programming.html).
...
##### Note
In particular, this avoids confusing macros with non-macro symbolic constants (see also [Don't use `ALL_CAPS` for enumerators](#renum-caps))
...
#### What do these guidelines aim to achieve? {#faq-aims}
...
#### When and where was this work first announced? {#faq-announced}
...
#### Who are the authors and maintainers of these guidelines? {#faq-maintainers}
...
#### Have these guidelines been approved by the ISO C++ standards committee? Do they represent the consensus of the committee? {#faq-iso}
...
#### If these guidelines are not approved by the committee, why are they under `github.com/isocpp`? {#faq-isocpp}
...
#### Will there be a C++98 version of these Guidelines? A C++11 version? {#faq-cpp98}
...
#### Do these guidelines propose new language features? {#faq-language-extensions}
...
#### What version of Markdown do these guidelines use? {#faq-markdown}
...
#### What is the GSL (guidelines support library)? {#faq-gsl}
...
#### Is [github.com/Microsoft/GSL](https://github.com/Microsoft/GSL) the GSL? {#faq-msgsl}
...
#### Why not supply an actual GSL implementation in/with these guidelines? {#faq-gsl-implementation}
We are reluctant to bless one particular implementation because we do not want to make people think there is only one, and inadvertently stifle parallel implementations. And if these guidelines included an actual implementation, then whoever contributed it could be mistakenly seen as too influential. We prefer to follow the long-standing approach of the committee, namely to specify interfaces, not implementations. But at the same time we want at least one implementation available; we hope for many.

#### Why weren't the GSL types proposed through Boost? {#faq-boost}
...
#### Has the GSL (guidelines support library) been approved by the ISO C++ standards committee? {#faq-gsl-iso}
...
#### If you're using the standard types where available, why is the GSL `span<char>` different from the `string_view` in the Library Fundamentals 1 Technical Specification and C++17 Working Paper? Why not just use the committee-approved `string_view`? {#faq-gsl-string-view}
...
#### Is `owner` the same as the proposed `observer_ptr`? {#faq-gsl-owner}
...
#### Is `stack_array` the same as the standard `array`? {#faq-gsl-stack-array}
...
#### Is `dyn_array` the same as `vector` or the proposed `dynarray`? {#faq-gsl-dyn-array}
...
### Define Copy, move, and destroy consistently {#sd-consistent}
##### Reason
???
...
(based off the glossary in [Principles and Practice using C++](https://www.stroustrup.com/programming.html))
...
## To-do {#s-unclassified}
::: {.subheading}
Unclassified proto-rules
:::
...
## Bibliography
- <a name="Abrahams01"></a> \[Abrahams01]:  D. Abrahams. [Exception-Safety in Generic Components](https://www.boost.org/community/exception_safety.html).
- <a name="Alexandrescu01"></a> \[Alexandrescu01]:  A. Alexandrescu. Modern C++ Design (Addison-Wesley, 2001).
- <a name="Cplusplus03"></a> \[C++03]:           ISO/IEC 14882:2003(E), Programming Languages — C++ (updated ISO and ANSI C++ Standard including the contents of (C++98) plus errata corrections).
- <a name="Cargill92"></a> \[Cargill92]:       T. Cargill. C++ Programming Style (Addison-Wesley, 1992).
- <a name="Cline99"></a> \[Cline99]:         M. Cline, G. Lomow, and M. Girou. C++ FAQs (2ndEdition) (Addison-Wesley, 1999).
- <a name="Dewhurst03"></a> \[Dewhurst03]:      S. Dewhurst. C++ Gotchas (Addison-Wesley, 2003).
- <a name="Henricson97"></a> \[Henricson97]:     M. Henricson and E. Nyquist. Industrial Strength C++ (Prentice Hall, 1997).
- <a name="Koenig97"></a> \[Koenig97]:        A. Koenig and B. Moo. Ruminations on C++ (Addison-Wesley, 1997).
- <a name="Lakos96"></a> \[Lakos96]:         J. Lakos. Large-Scale C++ Software Design (Addison-Wesley, 1996).
- <a name="Meyers96"></a> \[Meyers96]:        S. Meyers. More Effective C++ (Addison-Wesley, 1996).
- <a name="Meyers97"></a> \[Meyers97]:        S. Meyers. Effective C++ (2nd Edition) (Addison-Wesley, 1997).
- <a name="Meyers01"></a> \[Meyers01]:        S. Meyers. Effective STL (Addison-Wesley, 2001).
- <a name="Meyers05"></a> \[Meyers05]:        S. Meyers. Effective C++ (3rd Edition) (Addison-Wesley, 2005).
- <a name="Meyers15"></a> \[Meyers15]:        S. Meyers. Effective Modern C++ (O'Reilly, 2015).
- <a name="Murray93"></a> \[Murray93]:        R. Murray. C++ Strategies and Tactics (Addison-Wesley, 1993).
- <a name="Stroustrup94"></a> \[Stroustrup94]:    B. Stroustrup. The Design and Evolution of C++ (Addison-Wesley, 1994).
- <a name="Stroustrup00"></a> \[Stroustrup00]:    B. Stroustrup. The C++ Programming Language (Special 3rdEdition) (Addison-Wesley, 2000).
- <a name="Stroustrup05"></a> \[Stroustrup05]:    B. Stroustrup. [A rationale for semantically enhanced library languages](https://www.stroustrup.com/SELLrationale.pdf).
- <a name="Stroustrup13"></a> \[Stroustrup13]:    B. Stroustrup. [The C++ Programming Language (4th Edition)](https://www.stroustrup.com/4th.html). Addison-Wesley 2013.
- <a name="Stroustrup14"></a> \[Stroustrup14]:    B. Stroustrup. [A Tour of C++](https://www.stroustrup.com/Tour.html). Addison-Wesley 2014.
- <a name="Stroustrup15"></a> \[Stroustrup15]:    B. Stroustrup, Herb Sutter, and G. Dos Reis: [A brief introduction to C++'s model for type- and resource-safety](https://github.com/isocpp/CppCoreGuidelines/blob/master/docs/Introduction%20to%20type%20and%20resource%20safety.pdf).
- <a name="SuttHysl04b"></a> \[SuttHysl04b]:     H. Sutter and J. Hyslop. [Collecting Shared Objects](https://web.archive.org/web/20120926011837/http://www.drdobbs.com/collecting-shared-objects/184401839) (C/C++ Users Journal, 22(8), August 2004).
- <a name="SuttAlex05"></a> \[SuttAlex05]:      H. Sutter and  A. Alexandrescu. C++ Coding Standards. Addison-Wesley 2005.
- <a name="Sutter00"></a> \[Sutter00]:        H. Sutter. Exceptional C++ (Addison-Wesley, 2000).
- <a name="Sutter02"></a> \[Sutter02]:        H. Sutter. More Exceptional C++ (Addison-Wesley, 2002).
- <a name="Sutter04"></a> \[Sutter04]:        H. Sutter. Exceptional C++ Style (Addison-Wesley, 2004).
- <a name="Taligent94"></a> \[Taligent94]: Taligent's Guide to Designing Programs (Addison-Wesley, 1994).
