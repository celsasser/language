...
Several of the features that define modern C++ are additions layered on top of the C++98 core rather than replacements for it. This section houses them. We start with [`auto`](#auto-and-type-deduction), since it underpins how the others read, then move to the [lambda](#lambdas): an anonymous function you can write inline and pass around like a value. Other members of this cluster (structured bindings, [`constexpr`](#consteval-and-constinit)) get their own sub-sections later; the [`concepts`](#concepts) that constrain templates live under [Templates](#templates), since that is where they do their work.
