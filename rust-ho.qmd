...
[or](https://doc.rust-lang.org/std/option/enum.Option.html#method.or){target="_blank" rel="noopener noreferrer"}: returns `self` if it is `Some`. Otherwise, returns other. Useful for providing a fallback `Option`.
...
[or_else](https://doc.rust-lang.org/std/option/enum.Option.html#method.or_else){target="_blank" rel="noopener noreferrer"}: similar to `or`, but the fallback `Option` is computed by a closure f only if self is `None`. Useful for expensive fallback computations.
...
[unwrap_or](https://doc.rust-lang.org/std/option/enum.Option.html#method.unwrap_or){target="_blank" rel="noopener noreferrer"}: returns the contained `Some` value, or a provided default value if self is `None`. Use when `None` should produce a sensible default and the default is cheap.
...
[unwrap_or_else](https://doc.rust-lang.org/std/option/enum.Option.html#method.unwrap_or_else){target="_blank" rel="noopener noreferrer"}: returns the contained `Some` value, or computes it from a closure f if self is `None`. Useful when the default value is expensive to compute. Use when the fallback is expensive or requires logic — the closure only runs if the value is `None`.
...
- [is_some](https://doc.rust-lang.org/std/option/enum.Option.html#method.is_some){target="_blank" rel="noopener noreferrer"}: returns `true` if the `Option` is `Some`, `false` otherwise.
- [is_none](https://doc.rust-lang.org/std/option/enum.Option.html#method.is_none){target="_blank" rel="noopener noreferrer"}: returns `true` if the `Option` is `None`, `false` otherwise.
...
[map](https://doc.rust-lang.org/std/option/enum.Option.html#method.map){target="_blank" rel="noopener noreferrer"}: transforms the `Some` value into a new `Option` containing the result of applying a function `f`. If `self` is `None`, it remains `None`.
...
[and_then](https://doc.rust-lang.org/std/option/enum.Option.html#method.and_then){target="_blank" rel="noopener noreferrer"}: chains operations that themselves return an `Option`. If self is `Some(v)`, applies f to v and returns the resulting `Option`. If self is `None`, returns `None`. Prevents `Option<Option<T>>` (nesting).
...
[filter](https://doc.rust-lang.org/std/option/enum.Option.html#method.filter){target="_blank" rel="noopener noreferrer"}: returns `None` if the option is `None`, otherwise calls `predicate` with the wrapped value and returns `Some(t)` if `predicate` returns `true` and `None` if `predicate` returns `false`.
...
[ok_or](https://doc.rust-lang.org/std/option/enum.Option.html#method.ok_or){target="_blank" rel="noopener noreferrer"}: converts `Option<T>` into a `Result<T, E>`. If self is `Some(v)`, returns `Ok(v)`. If self is `None`, returns `Err(err)`. When you need to treat `None` as an error:
...
- `#`: This flag indicates that the “alternate” form of printing should be used. The alternate forms are:
	...
	See  [Formatting traits](https://doc.rust-lang.org/std/fmt/#formatting-traits){target="_blank" rel="noopener noreferrer"} for a description of what the `?`, `x`, `X`, `b`, and `o` flags do.
...
For more information, see [Module fmt](https://doc.rust-lang.org/std/fmt/){target="_blank" rel="noopener noreferrer"}.
...
::: {.explanation data-label="It looks Wrong"}
...
1. **Unit Variants:** Contain no data at all (like a traditional C-style enum).
2. **Tuple Variants:** Contain unnamed, ordered fields using parentheses: `Color(u8, u8, u8)`.
3. **Struct Variants:** Contain named fields enclosed in curly braces: `Move { x: i32, y: i32 }`.
...
Then you are golden, but you lose the explicit clarity of which `i32` represents the x-axis and which represents y-axis. So put on your big boy britches and get used to it. There is no way around it.
:::
...
For more information, see [Appendix C: Derivable Traits](https://doc.rust-lang.org/book/appendix-03-derivable-traits.html){target="_blank" rel="noopener noreferrer"} in *The Rust Programming Language*.
:::
