...
The qualification: Rust has bounds-checked array indexing in debug builds, and in release builds the checks are still present unless you use [`unsafe`](https://doc.rust-lang.org/reference/unsafe-keyword.html?highlight=unsafe#the-unsafe-keyword){target=_blank} indexing explicitly. In tight inner loops over arrays this can matter. C has no such checks at all. If you need to squeeze out that last few percent, Rust gives you the escape hatch (`unsafe`), but you have to ask for it deliberately. In practice, for most systems code, the difference does not appear outside of microbenchmarks.
...
### Crates
Before we define what a create is, it's important to understand what a *fundamental unit of compilation* is in Rust.
...
|----------|-------|--------|
| `+` | [`Add`](https://doc.rust-lang.org/std/ops/trait.Add.html) | `add(self, other: Self) -> Self` |
| `-` | [`Sub`](https://doc.rust-lang.org/std/ops/trait.Sub.html) | `sub(self, other: Self) -> Self` |
...
| `<`, `>`, `<=`, `>=` | [`PartialOrd`](https://doc.rust-lang.org/std/cmp/trait.PartialOrd.html) | `partial_cmp(&self, rhs: &Rhs) -> bool` |
...
### Functions
Functions are beautiful, reusible blocks of logic. [Wikipedia](https://en.wikipedia.org/wiki/Function_(computer_programming)) describes a **function** as "a callable unit of software logic that has a well-formed interface and behavior and can be invoked multiple times... Their primary purpose is to allow for the decomposition of a large and/or complicated problem into chunks that have relatively low cognitive load and to assign the chunks meaningful names (unless they are anonymous). Judicious application can reduce the cost of developing and maintaining software, while increasing its quality and reliability." Where would any language be without them?
