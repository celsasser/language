...
The following list is a top 30 crates capture from [Most popular Rust Libraries](https://lib.rs/std). When I finish this document, I'm going to put it in a time-capsule bury it.
...
| [thiserror](https://lib.rs/crates/thiserror) | derive(Error) |
...
| [clap](https://lib.rs/crates/clap) | efficient, and full-featured Command Line Argument Parser |
...
| [regex](https://lib.rs/crates/regex) | regular expressions for Rust. This implementation uses finite automata and guarantees linear time matching on all inputs. |
| [base64](https://lib.rs/crates/base64) | encodes and decodes base64 as bytes or utf8 |
| [log](https://lib.rs/crates/log) | A lightweight logging facade for Rust |
| [tracing](https://lib.rs/crates/tracing) | Application-level tracing for Rust |
| [tempfile](https://lib.rs/crates/tempfile) | managing temporary files and directories |
...
| [reqwest](https://lib.rs/crates/reqwest) | higher level HTTP client library |
| [libc](https://lib.rs/crates/libc) | Raw FFI bindings to platform libraries like libc |
...
| [toml](https://lib.rs/crates/toml) | A native Rust encoder and decoder of TOML-formatted files and streams. Provides implementations of the standard Serialize/Deserialize traits for TOML data to facilitate deserializing and serializing Rust structures. |
...
| [futures](https://lib.rs/crates/futures) | streams featuring zero allocations, composability, and iterator-like interfaces |
| [uuid](https://lib.rs/crates/uuid) | generate and parse UUIDs |
| [tracing-subscriber](https://lib.rs/crates/tracing-subscriber) | implementing and composing tracing subscribers |
| [bytes](https://lib.rs/crates/bytes) | Types and traits for working with bytes |
| [url](https://lib.rs/crates/url) | based on the WHATWG URL Standard |
...
| [getrandom](https://lib.rs/crates/getrandom) | A small cross-platform library for retrieving random data from system source |
| [indexmap](https://lib.rs/crates/indexmap) | A hash table with consistent order and fast iteration |
...
`use` is an extremely useful and flexible keyword that was described to me to an "alias injector". [The Rust Reference](https://doc.rust-lang.org/reference/items/use-declarations.html) has the following to say about `use`:
...
Many operators can be overloaded by implementing traits from [`std::ops`](https://doc.rust-lang.org/std/ops/index.html). For example:
...
In the following table we include some of A listers and indicate the operator, the trait (in `std::ops`) and the method signature. For a complete list, see [`std::ops`](https://doc.rust-lang.org/std/ops/index.html#traits).

| Operator | Trait | Method |
|----------|-------|--------|
| `+` | [`Add`](https://doc.rust-lang.org/std/ops/trait.Add.html) | `add(self, rhs: Rhs) -> Self` |
| `-` | [`Sub`](https://doc.rust-lang.org/std/ops/trait.Sub.html) | `sub(self, rhs: Rhs) -> Self` |
| `*` | [`Mul`](https://doc.rust-lang.org/std/ops/trait.Mul.html) | `mul(self, rhs: Rhs) -> Self` |
| `/` | [`Div`](https://doc.rust-lang.org/std/ops/trait.Div.html) | `div(self, rhs: Rhs) -> Self` |
| `%` | [`Rem`](https://doc.rust-lang.org/std/ops/trait.Rem.html) | `rem(self, rhs: Rhs) -> Self` |
| `==`, `!=` | [`PartialEq`](https://doc.rust-lang.org/std/cmp/trait.PartialEq.html) | `eq(&self, other: &Rhs) -> bool`, `ne(&self, other: &Rhs) -> bool` |
| `<`, `>`, `<=`, `>=` | [`PartialOrd`](https://doc.rust-lang.org/std/cmp/trait.PartialOrd.html) | `partial_cmp(&self, rhs: &Rhs) -> Option<Ordering>` |
| `[]` | [`Index`](https://doc.rust-lang.org/std/ops/trait.Index.html) | `index(&self, index: Idx) -> &Self::Output` |
| `[]` (mut) | [`IndexMut`](https://doc.rust-lang.org/std/ops/trait.IndexMut.html) | `index_mut(&mut self, index: Idx) -> &mut Self::Output` |
...
The operands are taken *by value* because addition is modeled as **consumption and transformation** — you take the inputs and produce a new output. For types where this makes sense (like integers), the values are [`Copy`](https://doc.rust-lang.org/std/marker/trait.Copy.html), so "moving" them is a no-op anyway. The compiler just copies the bits and you never notice.
...
|----------|--------|----------|-----------|----------------------------|
| [`String`](https://doc.rust-lang.org/std/string/struct.String.html) | Yes    | Yes      | Heap      | `std::string` in C++       |
| [`&str`](https://doc.rust-lang.org/std/primitive.str.html)   | No     | No       | Anywhere  | `std::string_view` in C++  |
...
[`String`](https://doc.rust-lang.org/std/string/struct.String.html) stores three things: a pointer to heap memory, a length (bytes used), and a capacity (bytes allocated). It is *not* null-terminated. The length field is what tells Rust where the string ends.

[`&str`](https://doc.rust-lang.org/std/primitive.str.html) is a **fat pointer**: a pointer into some UTF-8 buffer plus a length. It borrows that buffer — it owns nothing and cannot outlive the data it points into.
...
### Functions
Functions are beautiful, reusable blocks of logic. [Wikipedia](https://en.wikipedia.org/wiki/Function_(computer_programming)) describes a **function** as "a callable unit of software logic that has a well-formed interface and behavior and can be invoked multiple times... Their primary purpose is to allow for the decomposition of a large and/or complicated problem into chunks that have relatively low cognitive load and to assign the chunks meaningful names (unless they are anonymous). Judicious application can reduce the cost of developing and maintaining software, while increasing its quality and reliability." Where would any language be without them?
...
We cover [closures](https://doc.rust-lang.org/book/ch13-01-closures.html) in more detail [below](#closures).
